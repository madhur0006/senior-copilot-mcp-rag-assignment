...
The supplied **Alarm Management API application** and the accompanying reference document must be used as one of the primary source systems.

The reference API includes asset search, metadata, alarm retrieval, summaries, trends, correlation, rationalization, priority scoring, operator recommendations, controlled KPI calculation, authentication, trace metadata, pagination, and multi-step API chaining.
...
## 4. Your Assigned Use Case — Alarm Investigation and Procedure Guidance Copilot

### Business Scenario

Operators and reliability engineers need to investigate alarms by correlating asset metadata, active alarms, historical patterns, related assets, recommended actions, and operating procedures.

Build a copilot that performs an evidence-backed alarm investigation using MCP tools and document RAG.
...
Expose relevant Alarm Management API operations:
...
2. Resolve the asset name to an asset identifier.
3. Retrieve active or historical alarms through MCP.
4. Fetch asset metadata and related assets.
5. Run correlation or priority scoring.
6. Retrieve relevant document passages using RAG.
7. Compare API recommendations with document guidance.
8. Produce a grounded answer with citations and tool trace.

### GUI Expectations

- Chat interface
- Alarm summary panel
- Likely causes and recommendations
- Document citations
- Expandable MCP tool-call trace
